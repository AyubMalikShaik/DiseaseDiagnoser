...
def find_most_frequent_symptoms(G, symptom, top_n=7):
...
def find_strongest_connections(G, symptom, top_n=7):
    if symptom in G:
...
def display_strongly_connected_symptoms(G,symptom,count):
    ...
    return fig
# Example usage
# symptom = 'tongue bleeding'
# G = buildGraph()
# G = buildGraphWithWeights()
# print("All nodes in graph:", list(G.nodes))
# 
# if symptom in G.nodes:
#     print(f"✅ '{symptom}' exists in the graph.")
# else:
#     print(f"❌ '{symptom}' is NOT in the graph.")
# 
# fig=display_strongly_connected_symptoms(G,symptom,10)
# fig.show()
